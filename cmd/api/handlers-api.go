...
import (
	...
	"fmt"
	"net/http"
	"ryn-app/internal/cards"
	"ryn-app/internal/encryption"
	"ryn-app/internal/models"
	"ryn-app/internal/urlsigner"
	"ryn-app/internal/validator"
	"strconv"
	"strings"
	"time"
...
// Invoice desctribes the JSON payload sent to the microservice
type Invoice struct {
	ID        int       `json:"id"`
...
func (app *application) callInvoiceMicro(inv Invoice) error {
	url := "http://localhost:5555/invoice/create-and-send"
...
func (app *application) authenticateToken(r *http.Request) (*models.User, error) {
	...
	token := headerParts[1]
	if len(token) != 26 {
		return nil, errors.New("authentixation token wrong size")
	}
